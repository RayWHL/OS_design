...
gboolean get_mem_rate ()
{
    ...
    swap_total = atoi(mem_tmp[18]);
    swap_free = atoi(mem_tmp[19]);
...
gboolean print_time (gpointer label){
    ...
    char *time_info=strtok(time_buf,"\n");
    char string[100];
    sprintf(string,"Time:%s\tCPU:%.2lf%%\t内存:%.2lf%%",time_info,100*rate,100*mem_rate);
